<template>
  <div style="height: 100%; width: 100%" v-if="isVisible">
    <el-table :data="deleteGroupTableData" style="width: 100%; height: 90%" @selection-change="selectGroups">
      <el-table-column type="selection" width="55" />
      <el-table-column prop="uuid" label="Uuid" width="200" />
      <el-table-column prop="name" label="群名称" width="120" show-overflow-tooltip />
      <el-table-column prop="owner_id" label="群主id" width="200" show-overflow-tooltip />
    ...
    <div class="main-able-button-container">
      <el-button style="background-color: rgb(252, 210.9, 210.9);" @click="deleteGroups">删除/全部删除</el-button>
    </div>
  </div>
</template>
  ...
  setup(props) {
    ...
    const deleteGroups = async () => {
      try {
        ...
        const rsp = await axios.post(store.state.backendUrl + "/group/delete", req);
        console.log(rsp);
        getGroupInfoList();
      } catch (error) {
        console.log(error);
