<template>
  <div class="contactlist-container">
    <div class="contactlist-header">
      <el-input v-model="contactSearch" class="contact-search-input" placeholder="搜索联系人/群聊" size="small"
        suffix-icon="Search" />
      <div class="contactlist-header-right">
        <el-dropdown placement="bottom" trigger="click">
          <button class="create-group-btn">
            <svg t="1733664667695" class="create-group-icon" viewBox="0 0 1024 1024" version="1.1"
              xmlns="http://www.w3.org/2000/svg" p-id="2875" width="128" height="128">
              <path
            ...
            <div class="newcontact-modal-body">
              <el-scrollbar max-height="400px">
                <ul class="newcontact-list" style="list-style-type: none">
                  <li v-for="newContact in newContactList" :key="newContact.contact_id" class="newcontact-item">
                    <div style="
                        display: flex;
                        align-items: center;
                    ...
                    <el-dropdown placement="right" trigger="click">
                      ...
                      <template #dropdown>
                        <el-dropdown-menu>
                          ...
                          <el-dropdown-item @click="handleReject(newContact.contact_id)">
                            拒绝
                          </el-dropdown-item>
                          <el-dropdown-item @click="handleBlack(newContact.contact_id)">
                            拉黑
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </template>
        ...
        <SmallModal :isVisible="isApplyContactModalVisible">
          ...
          <template v-slot:body>
            <div class="modal-body">
              <el-form ref="formRef" :model="applyContactReq" label-width="100px" class="apply-contact-form">
                <el-form-item prop="name" label="用户/群聊id" :rules="[
                  {
                    required: true,
                    message: '此项为必填项',
                    trigger: 'blur',
                  ...
                  <el-input v-model="applyContactReq.contact_id" placeholder="请填写申请的用户/群聊id" />
                </el-form-item>
                <el-form-item prop="message" label="申请消息">
                  <el-input v-model="applyContactReq.message" placeholder="选填，填写更容易通过" type="textarea" show-word-limit
                    maxlength="100" :autosize="{ minRows: 3, maxRows: 3 }" />
                </el-form-item>
              </el-form>
            </div>
          </template>
          <template v-slot:footer>
            <div class="modal-footer">
              <el-button class="modal-close-btn" @click="closeApplyContactModal">
                完成
              </el-button>
            </div>
          </template>
          ...
          <template v-slot:body>
            <el-scrollbar max-height="300px" style="
              ...
              <div class="creatgroup-modal-body">
                <el-form ref="formRef" :model="createGroupReq" label-width="80px" class="demo-dynamic">
                  ...
                  <el-form-item prop="notice" label="群公告">
                    <el-input v-model="createGroupReq.notice" type="textarea" show-word-limit maxlength="500"
                      :autosize="{ minRows: 3, maxRows: 3 }" placeholder="选填" />
                  </el-form-item>
                  <el-form-item prop="add_mode" label="加群方式" :rules="[
                    {
                      required: true,
    ...
    <div class="contactlist-body">
      <div class="contactlist-user">
        <el-menu router unique-opened @open="handleShowUserList" @close="handleHideUserList">
          <el-sub-menu index="1">
            <template #title>
          ...
          <el-menu-item v-for="user in filteredContactUserList" :key="user.user_id" @click="handleToChatUser(user)"
            ...
            <el-dropdown trigger="contextmenu" class="contactlist-dropdown" placement="right">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item @click="handleCancelBlack(user)">解除拉黑</el-dropdown-item>
                  <!-- 其他菜单项 -->
                </el-dropdown-menu>
              </template>
            </el-dropdown>
        ...
        <el-menu router unique-opened @open="handleShowMyGroupList" @close="handleHideMyGroupList">
          <el-sub-menu index="2">
            <template #title>
        ...
        <el-menu router unique-opened @open="handleShowMyJoinedGroupList" @close="handleHideMyJoinedGroupList">
          <el-sub-menu index="3">
            <template #title>
...
export default {
  ...
  setup() {
    ...
    // 计算属性：根据 contactSearch 过滤列表
    const filteredContactUserList = computed(() => {
      if (!contactSearch.value) {
        return data.contactUserList
      }
      return data.contactUserList.filter(u =>
        u.user_name.includes(contactSearch.value)
      )
    })

    // 新增：创建的群过滤
    const filteredMyGroupList = computed(() => {
      if (!contactSearch.value) return data.myGroupList
      return data.myGroupList.filter(g =>
        g.group_name.includes(contactSearch.value)
      )
    })

    // 新增：加入的群过滤
    const filteredMyJoinedGroupList = computed(() => {
      if (!contactSearch.value) return data.myJoinedGroupList
      return data.myJoinedGroupList.filter(g =>
        g.group_name.includes(contactSearch.value)
      )
    })
    ...
    const handleCreateGroup = async () => {
      try {
        data.createGroupReq.owner_id = userInfo.value.uuid;
        if (data.fileList.length > 0) {
          data.createGroupReq.avatar = store.state.avatarPath + data.fileList[0].name;
          console.log(data.createGroupReq.avatar);
          uploadRef.value.submit();
        }
        const response = await axios.post(
          store.state.backendUrl + "/group/create",
          data.createGroupReq
        );
      } catch (error) {
        console.error(error);
    ...
    const handleApplyGroup = async () => {
      try {
        ...
        let rsp = await axios.post(
          store.state.backendUrl + "/group/check-add-mode",
          req
        );
        ...
        data.applyContactReq.owner_id = userInfo.value.uuid;
        rsp = await axios.post(
          store.state.backendUrl + "/contact/apply",
    ...
    const handleShowUserList = async () => {
      try {
        ...
        const getUserListRsp = await axios.post(
          store.state.backendUrl + "/contact/list",
          data.ownListReq
        );
    ...
    const handleShowMyGroupList = async () => {
      try {
        ...
        const loadMyGroupRsp = await axios.post(
          store.state.backendUrl + "/group/load-my",
          data.ownListReq
        );
    ...
    const handleToChatUser = async (user) => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/session/check-allowed",
          req
        );
        if (rsp.data.code == 200) {
    ...
    const handleNewContactList = async () => {
      try {
        data.ownListReq.owner_id = userInfo.value.uuid;
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/new-list",
          data.ownListReq
        );
        console.log(rsp);
    ...
    const handleEnterDirectly = async (groupId) => {
      try {
        const req = {
          owner_id: groupId,
          contact_id: userInfo.value.uuid,
        };
        const rsp = await axios.post(
          store.state.backendUrl + "/group/enter",
          req
        );
        console.log(rsp);
        if (rsp.data.code == 200) {
    ...
    const handleBlack = async (contactId) => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/black-apply",
          req
        );
        if (rsp.data.code == 200) {
          ElMessage.success(rsp.data.message);
    ...
    const handleCancelBlack = async (user) => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/cancel-black",
          req
        );
        if (rsp.data.code == 200) {
          ElMessage.success(rsp.data.message);
