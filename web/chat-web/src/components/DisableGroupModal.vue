<template>
  <div style="height: 100%; width: 100%" v-if="isVisible">
    <el-table :data="disableGroupTableData" style="width: 100%; height: 90%" @selection-change="selectGroups">
      <el-table-column type="selection" width="55" />
      <el-table-column prop="uuid" label="Uuid" width="200" />
      <el-table-column prop="name" label="群名称" width="120" show-overflow-tooltip />
      <el-table-column prop="owner_id" label="群主id" width="120" show-overflow-tooltip />
      <el-table-column prop="status" label="禁用状态" width="120">
        <template #default="scope">
          <el-button type="default" v-if="scope.row.status == 0">否</el-button>
    ...
    <div class="main-able-button-container">
      <el-button style="background-color: rgb(252, 210.9, 210.9); margin-left: 20px;"
        @click="setGroupsStatus(1)">禁用/全部禁用</el-button>
      <el-button style="background-color: rgb(252, 210.9, 210.9);" @click="setGroupsStatus(0)">启用/全部启用</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props) {
    ...
    // 监视 isVisible，从 false→true 时拉数据
    watch(
      () => props.isVisible,
      visible => { if (visible) getGroupInfoList() },
      { immediate: false }
    );
    ...
    const getGroupInfoList = async () => {
      try {
        const rsp = await axios.post(
          store.state.backendUrl + "/group/info-list"
        );
    ...
    const setGroupsStatus = async (status) => {
      try {
        ...
        const rsp = await axios.post(store.state.backendUrl + "/group/set-status", req);
        console.log(rsp);
        getGroupInfoList();
      } catch (error) {
        console.log(error);
