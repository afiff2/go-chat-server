<template>
  <div style="height: 100%; width: 100%" v-if="isVisible">
    <el-table :data="setAdminTableData" style="width: 100%; height: 90%" @selection-change="selectUsers">
    ...
    <div class="main-able-button-container">
      <el-button style="background-color: rgb(252, 210.9, 210.9); margin-left: 20px;"
        @click="setAdmin(1)">设置/全部设置为管理员</el-button>
      <el-button style="background-color: rgb(252, 210.9, 210.9)" @click="setAdmin(0)">取消/全部取消管理员</el-button>
    </div>
  </div>
</template>
  ...
  setup(props) {
    ...
    const setAdmin = async (isAdmin) => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/user/set-admin", req);
        console.log(rsp);
        getUserList();
      } catch (error) {
        console.log(error);
