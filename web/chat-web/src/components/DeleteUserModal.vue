<template>
  <div style="height: 100%; width: 100%" v-if="isVisible">
    <el-table :data="deleteUserTableData" style="width: 100%; height: 90%" @selection-change="selectUsers">
    ...
    <div class="main-able-button-container">
      <el-button style="background-color: rgb(252, 210.9, 210.9);" @click="deleteUsers">删除/全部删除</el-button>
    </div>
  </div>
</template>
  ...
  setup(props) {
    ...
    const deleteUsers = async () => {
      try {
        ...
        const rsp = await axios.post(store.state.backendUrl + "/user/delete", req);
        console.log(rsp);
        getUserList();
      } catch (error) {
        console.log(error);
