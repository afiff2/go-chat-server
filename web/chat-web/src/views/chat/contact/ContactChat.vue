        ...
        <el-aside class="aside-container">
          ...
          <div class="sessionlist-container">
            <div class="sessionlist-header">
              <el-input v-model="contactSearch" class="contact-search-input" placeholder="搜索会话" size="small"
                suffix-icon="Search" />
            </div>
            <div class="contactlist-body">
              <div class="contactlist-user">
                <el-menu router unique-opened @open="handleShowUserSessionList" @close="handleHideUserSessionList">
                  <el-sub-menu index="1">
                    <template #title>
                ...
                <el-menu router unique-opened @open="handleShowGroupSessionList" @close="handleHideGroupSessionList">
                  <el-sub-menu index="1">
                    <template #title>
        ...
        <el-container class="chat-container">
          <el-header>
            <div class="chat-title" v-if="contactInfo.contact_avatar">
              <img :src="contactInfo.contact_avatar" style="width: 40px; height: 40px; margin-right: 10px" />
            ...
            <div class="chat-title-right">
              <Modal :isVisible="isUserContactInfoModalVisible">
                <template v-slot:header>
                  <div class="userinfo-modal-quit-btn-container">
                    <button class="userinfo-modal-quit-btn" @click="quitUserContactInfoModal">
                      <el-icon>
                        <Close />
                      </el-icon>
                    </button>
                ...
                <template v-slot:body>
                  <el-descriptions direction="vertical" border class="modal-list" size="small">
                    <el-descriptions-item :rowspan="2" :width="120" label="头像" align="center">
                      <el-image style="width: 100px; height: 100px" :src="contactInfo.contact_avatar" />
                    ...
                    <el-descriptions-item label="生日" :span="1" :width="140">{{ contactInfo.contact_birthday }}
                    </el-descriptions-item>
              ...
              <Modal :isVisible="isGroupContactInfoModalVisible">
                <template v-slot:header>
                  <div class="groupcontactinfo-modal-quit-btn-container">
                    <button class="groupcontactinfo-modal-quit-btn" @click="quitGroupContactInfoModal">
                      <el-icon>
                        <Close />
                      </el-icon>
                    </button>
              ...
              <el-dropdown placement="bottom" trigger="click">
                ...
                <template #dropdown>
                  ...
                  <el-dropdown-menu v-else-if="contactInfo.contact_id[0] === 'G'">
                    <el-dropdown-item @click="showGroupContactInfoModal">群聊信息</el-dropdown-item>
                    <el-dropdown-item v-if="contactInfo.contact_owner_id == userInfo.uuid"
                    ...
                    <el-dropdown-item v-if="contactInfo.contact_owner_id == userInfo.uuid"
                      @click="showAddGroupModal">加群申请</el-dropdown-item>
                    <el-dropdown-item @click="preToDeleteSession">删除该会话</el-dropdown-item>
                    <el-dropdown-item v-if="contactInfo.contact_owner_id == userInfo.uuid"
                      @click="handleDismissGroup">解散群聊</el-dropdown-item>
                    <el-dropdown-item v-if="contactInfo.contact_owner_id != userInfo.uuid"
                      @click="handleLeaveGroup">退出群聊</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
              <Modal :isVisible="isUpdateGroupInfoModalVisible">
                <template v-slot:header>
                  <div class="updategroupinfo-modal-quit-btn-container">
                    <button class="updategroupinfo-modal-quit-btn" @click="quitUpdateGroupInfoModal">
                      <el-icon>
                        <Close />
                      </el-icon>
                    </button>
                ...
                <template v-slot:body>
                  <el-scrollbar max-height="255px" style="
                      width: 400px;
                      height: 255px;
                      display: flex;
                      align-items: center;
                    ...
                    <div class="modal-body">
                      <el-form ref="formRef" :model="updateGroupInfo" label-width="80px">
                        <el-form-item prop="name" label="群名称" :rules="[
                          {
                            min: 3,
                            max: 10,
                          ...
                          <el-input v-model="updateGroupInfo.name" placeholder="选填" />
                        </el-form-item>
                        ...
                        <el-form-item prop="notice" label="群公告">
                          <el-input v-model="updateGroupInfo.notice" type="textarea" show-word-limit maxlength="500"
                            :autosize="{ minRows: 3, maxRows: 3 }" placeholder="选填" />
                        </el-form-item>
                        <el-form-item prop="avatar" label="群头像">
                          <el-upload v-model:file-list="avatarList" ref="uploadAvatarRef" :auto-upload="false"
                            :action="uploadAvatarPath" :on-success="handleAvatarUploadSuccess"
                ...
                <template v-slot:footer>
                  <div class="updategroupinfo-modal-footer">
                    <el-button style="background-color: rgb(252, 210.9, 210.9)" @click="closeUpdateGroupInfoModal">
                      完成
                    </el-button>
                  </div>
                </template>
              ...
              <Modal :isVisible="isRemoveGroupMemberModalVisible">
                <template v-slot:header>
                  <div class="removegroupmember-modal-quit-btn-container">
                    <button class="removegroupmember-modal-quit-btn" @click="quitRemoveGroupMemberModal">
                      <el-icon>
                        <Close />
                      </el-icon>
                    </button>
                ...
                <template v-slot:body>
                  <span style="
                      ...
                      margin-bottom: 5px;
                    ">群组成员：</span>
                  <el-scrollbar max-height="400px" style="height: 300px; width: 350px">
                    <div class="modal-body">
                      <ul style="list-style-type: none" class="removegroupmembers-list">
                        <li v-for="groupMember in groupMemberList" :key="groupMember.user_id"
                          class="removegroupmembers-item">
                          <div style="display: flex; align-items: center">
                            <el-image :src="groupMember.avatar" class="removegroupmembers-item-avatar" />
                            <span class="removegroupmembers-item-name">{{
                              groupMember.nickname
                            }}</span>
                          </div>
                          <input type="checkbox" :value="groupMember.user_id" v-model="selectedGroupMembers"
                ...
                <template v-slot:footer>
                  <div style="
                    ...
                    ">
                    <el-button class="removegroupmembers-button" @click="handleRemoveGroupMembers">移除所选人员</el-button>
                  </div>
                </template>
              </Modal>
                  ...
                  <div class="addGroup-modal-body">
                    <el-scrollbar max-height="400px">
                      <ul class="addGroup-list" style="list-style-type: none">
                        <li v-for="addGroup in addGroupList" :key="addGroup.contact_id" class="addGroup-item">
                          <div style="
                              display: flex;
                              align-items: center;
                              justify-content: center;
                            ">
                            <img :src="addGroup.contact_avatar" style="
                                width: 30px;
                                height: 30px;
                                margin-right: 10px;
                              " />
                          ...
                          <el-dropdown placement="right" trigger="click">
                            ...
                            <template #dropdown>
                              <el-dropdown-menu>
                                <el-dropdown-item @click="handleAgree(addGroup.contact_id)">同意</el-dropdown-item>
                                <el-dropdown-item @click="handleReject(addGroup.contact_id)">
                                  拒绝
                                </el-dropdown-item>
                              </el-dropdown-menu>
                            </template>
                ...
                <div v-for="(messageItem, index) in messageList" :key="index" class="message-item">
                  <div v-if="messageItem.send_id != userInfo.uuid &&
                    ...
                    <div class="left-message-left">
                      <el-image :src="messageItem.send_avatar" style="
                          width: 40px;
                          height: 40px;
                          margin-left: 10px;
                          margin-right: 10px;
                      ...
                      <div class="left-message-file-container">
                        ...
                        <div class="left-message-file-download">
                          <el-button style="
                              ...
                              margin-top: 20px;
                            " size="small" @click="downloadFile(messageItem.file_name)">
                            下载
                          </el-button>
                        </div>
                      </div>
                  ...
                  <div style="
                    ...
                    <div v-if="messageItem.send_id == userInfo.uuid &&
                      ...
                      <div class="right-message-right">
                        <el-image :src="userInfo.avatar" style="
                            width: 40px;
                            height: 40px;
                            margin-left: 10px;
                            margin-right: 10px;
              ...
              <div class="tool-bar-left">
                <el-tooltip effect="customized" content="表情包" placement="top" :hide-after="0" :enterable="false">
                  <button class="image-button" @click="
                    ...
                    ">
                    <svg t="1733502796507" class="sticker-icon" viewBox="0 0 1024 1024" version="1.1"
                      xmlns="http://www.w3.org/2000/svg" p-id="1555" width="128" height="128">
                      <path
                  ...
                  <button class="image-button">
                    <el-upload v-model:file-list="fileList" ref="uploadRef" :auto-upload="true" :show-file-list="false"
                      :action="uploadPath" :on-success="handleUploadSuccess" :before-upload="beforeFileUpload" style="
                        ...
                        justify-content: center;
                      ">
                      <svg t="1733503065264" class="upload-icon" viewBox="0 0 1024 1024" version="1.1"
                        xmlns="http://www.w3.org/2000/svg" p-id="2430" width="128" height="128">
                        <path
                  ...
                  <button class="image-button" @click="showAVContainerModal">
                    <svg t="1733503700535" class="av-icon" viewBox="0 0 1024 1024" version="1.1"
                      ...
                      <path
                        ...
                        fill="#666666" p-id="4493"></path>
                    </svg>
                  </button>
                </el-tooltip>
                <div class="video-modal-overlay" v-show="isAVContainerModalVisible">
                  <div class="video-modal-content">
                    ...
                    <div class="video-modal-footer">
                      <el-button class="video-modal-footer-btn" @click="startCall(true)">发起通话</el-button>
                      <el-button class="video-modal-footer-btn" @click="startCall(false)">接收通话</el-button>
                      <el-button class="video-modal-footer-btn" @click="rejectCall">拒绝通话</el-button>
                      <el-button class="video-modal-footer-btn" @click="sendEndCall">挂断通话</el-button>
                      <el-button class="video-modal-footer-btn" @click="closeAVContainerModal">退出聊天室</el-button>
                    </div>
                  </div>
                </div>
              </div>
...
<script>
import { reactive, toRefs, watch, ref, nextTick, computed } from "vue";
...
export default {
  ...
  setup() {
    ...
    const data = reactive({
      ...
      addGroupList: [],
      uploadRef: null,
      uploadPath: store.state.backendUrl + "/message/upload-file",
      fileList: [],
      uploadAvatarRef: null,
      uploadAvatarPath: store.state.backendUrl + "/message/upload-avatar",
      avatarList: [],
    ...
    const getChatContactInfo = async (id) => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/info",
    ...
    const getSessionId = async (contactId) => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/session/open",
          req
        );
    ...
    const getMessageList = async () => {
      try {
        ...
        console.log(req);
        const rsp = await axios.post(
          store.state.backendUrl + "/message/list",
          req
        );
        if (rsp.data.data) {
    ...
    const getGroupMessageList = async () => {
      try {
        ...
        console.log(req);
        const rsp = await axios.post(
          store.state.backendUrl + "/message/group-list",
          req
        );
        if (rsp.data.data) {
          for (let i = 0; i < rsp.data.data.length; i++) {
    ...
    const initChat = async (contactId) => {
      if (!contactId) return
      await getChatContactInfo(contactId);
      await getSessionId(contactId);
        ...
        if (message.type != 3) {
          if (
            ...
            (message.receive_id[0] == "U" &&
              message.receive_id == userInfo.value.uuid) ||
            // 自己发送的消息
          ...
          if (messageAVdata.messageId === "CURRENT_PEERS") {
            ...
            console.log("接收到PEER_LEAVE消息：", userInfo.value.uuid);
            receiveEndCall();
    ...
    const handleAddGroupList = async () => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/add-group-list",
          req
        );
    ...
    const deleteSession = async () => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/session/delete",
          req
        );
    ...
    const deleteContact = async () => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/delete",
          req
        );
        console.log(rsp.data);
      } catch (error) {
    ...
    const blackContact = async () => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/black",
          req
        );
        console.log(rsp.data);
      } catch (error) {
    ...
    const scrollToBottom = () => {
      nextTick(() => {
        ...
        if (el && sb) {
          sb.setScrollTop(el.scrollHeight)
        }
      })
    }
    ...
    const handleAgree = async (contactId) => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/pass-apply",
          req
        );
        console.log(rsp);
        if (rsp.data.code == 200) {
    ...
    const handleReject = async (contactId) => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/contact/refuse-apply",
          req
        );
        console.log(rsp);
        if (rsp.data.code == 200) {
    ...
    const handleLeaveGroup = async () => {
      try {
        const req = {
          user_id: userInfo.value.uuid,
          group_id: data.contactInfo.contact_id,
        };
        const rsp = await axios.post(
          store.state.backendUrl + "/group/leave",
          req
        );
        if (rsp.data.code == 200) {
          ElMessage.success(rsp.data.message);
    ...
    const handleDismissGroup = async () => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/group/dismiss",
          req
        );
        if (rsp.data.code == 200) {
          ElMessage.success(rsp.data.message);
    ...
    const handleUpdateGroupInfo = async () => {
      try {
        ...
        const rsp = await axios.post(
          store.state.backendUrl + "/group/update",
    ...
    const getGroupMemberList = async () => {
      ...
      try {
        const rsp = await axios.post(
          store.state.backendUrl + "/group/members",
          req
        );
        console.log(rsp);
        if (rsp.data.code == 200) {
    ...
    const handleRemoveGroupMembers = async () => {
      ...
      try {
        const rsp = await axios.post(
          store.state.backendUrl + "/group/remove-members",
          req
        );
        console.log(rsp);
        if (rsp.data.code == 200) {
