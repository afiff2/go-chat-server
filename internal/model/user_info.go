...
type UserInfo struct {
	...
	Uuid          string         `gorm:"column:uuid;uniqueIndex;type:char(36);comment:用户唯一id"`
	Nickname      string         `gorm:"column:nickname;type:varchar(20);not null;comment:昵称"`
	Telephone     string         `gorm:"column:telephone;index:idx_telephone,unique;not null;type:char(11);comment:电话"`
	...
	Password      string         `gorm:"column:password;type:varchar(255);not null;comment:加密后的密码(bcrypt)"`
